...
class DataWranglingAgent(BaseAgent):
    ...
    df = pd.read_csv("https://raw.githubusercontent.com/daniellopez882/ai-data-science-team/refs/heads/master/data/churn_data.csv")
            ...
            try:
                ...
                wrangling_summary = "\n".join(
                    [
                        ...
                        f"Rows: {rows_before} -> {rows_after} (Î” {rows_after - rows_before})",
                        f"Columns: {len(cols_before)} -> {len(cols_after)} (Î” {len(cols_after) - len(cols_before)})",
